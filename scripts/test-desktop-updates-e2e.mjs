...
import { assertEqual, assertStatus, fetchJson, fetchRaw, startApi, waitForPort } from './e2e.mjs'
...
async function main() {
    try {
        ...
        const empty = await fetchRaw('/api/desktop/darwin-aarch64/0.1.1', {}, apiUrl)
        ...
        const current = await fetchRaw('/api/desktop/darwin-aarch64/0.1.2', {}, apiUrl)
        assertStatus(current, 204, 'current client should not receive same-version update')

        const newer = await fetchJson('/api/desktop/darwin-aarch64/0.1.1', {}, apiUrl)
        ...
        const fullManifest = await fetchJson('/api/desktop', {}, apiUrl)
        assertEqual(
        ...
        const artifact = await fetchRaw(`/api/desktop/download/${encodeURIComponent(artifactName)}`, {}, apiUrl)
        assertStatus(artifact, 200, 'artifact download status')
        assertEqual(await artifact.text(), artifactBody, 'artifact download body')

        const missing = await fetchRaw('/api/desktop/download/missing.tar.gz', {}, apiUrl)
